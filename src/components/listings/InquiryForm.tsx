...
export default function ListingForm({ existing }: { existing?: Listing }) {
  ...
  const removeHighlight = (i: number) => {
    set('highlights', (form.highlights ?? []).filter((_: any, idx: number) => idx !== i))
  }
  ...
  const save = async (status?: string) => {
    ...
    if (existing) {
      await supabase.from('listings').update(payload as any).eq('id', existing.id)
    } else {
      await supabase.from('listings').insert(payload as any)
    }
    setSaving(false)
